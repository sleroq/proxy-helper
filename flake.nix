{
  ...
  in {
    ...
    packages = eachSystem (system: let sb = package nixpkgs.legacyPackages.${system}; in { inherit sb; default = sb; });
    devShells = eachSystem (system: { default = nixpkgs.legacyPackages.${system}.mkShell {
      packages = with nixpkgs.legacyPackages.${system}; [ go golangci-lint sing-box ];
    }; });
  };
}
